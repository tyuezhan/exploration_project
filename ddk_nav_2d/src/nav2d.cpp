...
nav2d::nav2d(){
    ...
    global_traj_marker_pub_ = nh.advertise<visualization_msgs::MarkerArray>("global_trajectory", 1, true);
    initial_traj_yaw_ = 0.0;
    nh.param("dt", dt_, 0.5);
    nh.param("num_opt_points", num_opt_points_, 7);
    nh.param("size", edrb_size_, 2.0);
                    ...
                    case EXPL_TARGET_SET:
                        ...
                        if (lineTrackerStatus == NAV_ST_IDLE){
                            ...
                            if(mCurrentMap.getCoordinates(goal_x,goal_y,mGoalPoint)){
                                ...
                                geometry_msgs::Quaternion goalQuaternion = tf::createQuaternionMsgFromYaw(yaw);
                                ...
                                // geometry_msgs::Quaternion startQuaternion = tf::createQuaternionMsgFromYaw(yaw_);
                                ...
                                // bool ret = getJpsTraj(local_time_, start, goal);
...
{
    ROS_INFO("enter update tracking path");
  global_traj_ = traj;
