...
class Nav2D:
    def __init__(self):
        ...
        self.explore_action_topic_ = rospy.get_param("~explore_action_topic", NAV_EXPLORE_ACTION)
        ...
        # Subscriber
        self.map_subscriber_ = rospy.Subscriber("projected_map", OccupancyGrid, self.mapSubscriberCB, queue_size=5)
        self.pose_subscriber_ = rospy.Subscriber("odom", Odometry, self.poseSubscriberCB, queue_size=10)
        ...
        # Services
        self.srv_transition_= rospy.ServiceProxy("trackers_manager/transition", Transition)
        self.jps_service_client_= rospy.ServiceProxy("jps_plan_service", GetPlan)

        # Action client
        self.line_tracker_min_jerk_client_ = actionlib.SimpleActionClient("trackers_manager/line_tracker_min_jerk/LineTracker", LineTrackerAction)
        ...
        self.traj_tracker_client_ = actionlib.SimpleActionClient("trackers_manager/trajectory_tracker/TrajectoryTracker", TrajectoryTrackerAction)
        ...
        # Action Server
        self.explore_action_server_ = actionlib.SimpleActionServer(self.explore_action_topic_, ExploreAction, execute_cb=self.receiveExploreGoal, auto_start = False)
                ...
                if self.preparePlan():
                    ...
                    if result == EXPL_TARGET_SET:
                        ...
                        if recheck or not moving:
                            # rospy.logwarn("Here!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! {}".format(self.goal_point_ ))
                            goal_x, goal_y = self.current_map_.getCoordinates(self.goal_point_)
                            # rospy.logwarn("Goal X Goal Y {}, {}".format(goal_x, goal_y))
                            if goal_x != False or goal_y != False:
                                ...
                                if recheck and moving:
                                    rospy.loginfo("Enter recheck exploration goal point.")
                                ...
                                yaw = math.atan2(self.pos_[1] - map_goal_y, self.pos_[0] - map_goal_x)
                                yaw += PI
                                if yaw < -PI:
                                    yaw += 2*PI
                                if yaw > PI:
                                ...
                                if not track_path:
                                    ...
                                    while True:
                                        ...
                                        if self.line_tracker_status_ == NAV_ST_TURNING:
                                            self.goTo(self.pos_[0], self.pos_[1], self.pos_[2], yaw, 0.0, 0.0, False);
                                        loop_rate.sleep()
                                
                                # rospy.logwarn("goal: {}, {}, {}, {}".format(map_goal_x, map_goal_y, map_goal_z, yaw))
                                # rospy.logwarn("current: {}, {}, {}".format(self.pos_[0], self.pos_[1], self.pos_[2]))
                                goal_value = self.current_map_.getData(self.goal_point_)
                                # rospy.logwarn("Goal value is: {}".format(goal_value))

                                distance = math.sqrt((map_goal_x - self.pos_[0])**2 + (map_goal_y - self.pos_[1])**2 + (map_goal_z - self.pos_[2])**2)
                                ...
                                try:
                                    transformStamped = self.tfBuffer_.lookup_transform(self.map_frame_, self.robot_frame_, rospy.Time(0))
                                except:
                                    rospy.loginfo("Couldn't get current odom transform")
                                    return
                                translation = transformStamped.transform.translation # translation.x, y, z
                                
                                # Check what is the expected goal yaw.
                                goal_yaw = self.getGoalHeading(self.goal_point_)
                                if goal_yaw < 0:
                                    rospy.logerr("Goal heading -1.")
                                
                                tf2_quaternion = tf_conversions.transformations.quaternion_from_euler(0, 0, goal_yaw)
                                goal = PoseStamped()
                                ...
                                # rospy.logwarn("Goal in main loop : {}".format(goal))
                                # DEBUG: potential issue here
                                ret = self.getJpsTraj(local_time, translation, goal, track_path)
    ...
    def goTo(self, x, y, z, yaw, v_des, a_des, relative):
        ...
        self.line_tracker_min_jerk_client_.send_goal(goal, done_cb=self.lineTrackerDoneCB)
    ...
    def trackPath(self, planned_path, method):
        if method:
            ...
            self.track_path_action_client_.send_goal(path_goal, done_cb=self.trackPathDoneCB)
            ...
            self.traj_tracker_client_.send_goal(path_goal, done_cb=self.trajTrackerDoneCB)
    ...
    def preparePlan(self):
        ...
        rospy.loginfo("prepare plan ready, set Startpoint as {}".format(self.start_point_))
        
        # debug
        current_x, current_y = self.current_map_.getCoordinates(self.start_point_)
        current_world_x = self.current_map_.getOriginX() + (current_x + 0.5) * self.current_map_.getResolution()
        current_world_y = self.current_map_.getOriginY() + (current_y + 0.5) * self.current_map_.getResolution()
        my_odom = PoseStamped()
