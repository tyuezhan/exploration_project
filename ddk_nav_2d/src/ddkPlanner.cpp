...
bool ddkPlanner::updateOctomap(octomap_msgs::Octomap msg) {
  ...
  ROS_INFO("ddk planner Octomap Updated. Resolution: %f, minX: %f, minY: %f, "
           "minZ: %f, maxX: %f, maxY: %f, maxZ: %f",
           resolution, minX, minY, minZ, maxX, maxY, maxZ);
...
void ddkPlanner::displayData() {
  for (octomap::OcTree::leaf_iterator it = octoTree->begin_leafs(),
                                      end = octoTree->end_leafs();
       it != end; ++it) {
    ROS_INFO("Node center: %f, %f, %f, %f", it.getX(), it.getY(), it.getZ(),
             it->getValue());
  }
}
...
bool ddkPlanner::isFrontier(Vec3 pos) {
  ...
  if (isFree(pos)) {
    ...
    for (int i = 0; i < 26; i++) {
      ...
      node = octoTree->search((double)neighbors[i](0), (double)neighbors[i](1),
                              (double)neighbors[i](2));
      if (node == NULL) {
        ROS_INFO("Neighbor of pos %f, %f, %f is unknown", neighbors[i](0),
  ...
  while (!queue.empty()) {
    ...
    if (isFrontier(currentPos)) {
      ...
      for (int i = 0; i < 26; i++) {
        ...
        if (it == checkedPos.end() && isFree(neighbors[i]) &&
            isInMap(neighbors[i])) {
