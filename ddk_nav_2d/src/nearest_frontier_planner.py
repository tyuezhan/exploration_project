...
class FrontierPlanner:
  def __init__(self, map_frame):
    ...
    self.path_publisher = rospy.Publisher(
        "frontier_search_path", Marker, queue_size=5)
  ...
  def index_to_pos(self, ind):
    ...
    world_x = self.map_in_.getOriginX() + (cell_x + 0.5) * \
        self.map_in_.getResolution()
    world_y = self.map_in_.getOriginY() + (cell_y + 0.5) * \
        self.map_in_.getResolution()
